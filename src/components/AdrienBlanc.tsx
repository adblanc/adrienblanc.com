...
function animateSgv(
  ...
  for (let i = 0; i < logoPaths.length; i++) {
    logoPaths[i].style.stroke = theme === "dark" ? "white" : "#2d3748";
...
const AdrienBlanc: React.FC<unknown> = () => {
  ...
  useEffect(() => {
    ...
    animateSgv(idLeft, 0, getDelayByLength(logoOneLength), theme);
    animateSgv(idRight, 0, getDelayByLength(logoTwoLength), theme);
  }, [theme]);

  return (
    <Link href="/">
      <a className="text-center flex justify-center space-x-3">
        <svg
          id={idLeft}
          className={svgClass}
          viewBox="0 0 488 107"
