    ...
    <section className="flex flex-row justify-center border-b border-t border-gray-300">
      {FILTERS.map(({ title, value }) => (
        <button
          ...
          } rounded-full m-2 py-2 px-3 text-xs cursor-pointer uppercase tracking-wider font-bold focus:outline-none border-2 border-gray-800 shadow-sm hover:shadow-md hover:bg-gray-800 hover:text-gray-200 dark:hover:bg-gray-100 dark:hover:text-gray-900 transition ease-in-out duration-100`}
        >
          {title}
        </button>
      ))}
